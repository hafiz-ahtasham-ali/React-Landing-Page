    ...
    return (
      <div className="relative w-full h-[483px] flex justify-start items-center bg-[#F5F7FA] text-[#4D4D4D] pt-10">
        ...
        <div className="w-1/3 flex justify-center">
          <img src={Image_1} alt={"title"} className="w-[442px] h-[433px]" />
        </div>
  
        {/* Right Side - Content (Left-Aligned) */}
        <div className="w-2/3 pl-10 text-left">
          {/* <h2 className="text-4xl font-bold text-gray-800">{"title"}</h2> */}
          <p className="text-[#717171] text-lg mt-2">Maecenas dignissim justo eget nulla rutrum molestie. Maecenas lobortis sem dui, vel rutrum risus tincidunt ullamcorper. Proin eu enim metus. Vivamus sed libero ornare, tristique quam in, gravida enim. Nullam ut molestie arcu, at hendrerit elit. Morbi laoreet elit at ligula molestie, nec molestie mi blandit. Suspendisse cursus tellus sed augue ultrices, quis tristique nulla sodales. Suspendisse eget lorem eu turpis vestibulum pretium. Suspendisse potenti. Quisque malesuada enim sapien, vitae placerat ante feugiat eget. Quisque vulputate odio neque, eget efficitur libero condimentum id. Curabitur id nibh id sem dignissim finibus ac sit amet magna.</p>
          <p className="text-[#4CAF4F] text-lg mt-2 font-semibold">Tim Smith</p>
          <p className="text-[#89939E] text-lg mt-2">British Dragon Boat Racing Association</p>
          <div className="flex justify-left space-x-15 mt-6">
            {logos.map((logo, index) => (
            <img key={index} src={logo} alt={`Logo_${index + 1}`} className="w-12 h-12" />
            ))}
            <button className="text-[#4CAF4F] px-6 py-3 hover:text-[#529463] font-bold">
                Meet all customers
                <span>→</span>
            </button>
        </div>
